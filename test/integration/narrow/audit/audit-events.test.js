...
describe('Integration - audit events conform to the fcp-audit-publisher schema', () => {
  ...
  test('row 2: document/created emitted with metadataId after document attachment', async () => {
    ...
    // addMetadataToExistingCase calls fetchOnlineSubmissionActivityIdOrThrow,
    // which in turn needs getOnlineSubmissionActivityId to resolve.
    const { getOnlineSubmissionActivityId } = await import('../../../../src/repos/crm.js')
    getOnlineSubmissionActivityId.mockResolvedValue({ onlineSubmissionActivityId: 'osa-1' })
  ...
  test('the full create-then-attach flow emits document/created twice with a shared correlationid', async () => {
    ...
    const { getOnlineSubmissionActivityId } = await import('../../../../src/repos/crm.js')
    getOnlineSubmissionActivityId.mockResolvedValue({ onlineSubmissionActivityId: 'osa-3' })
